readH5_QC_filter_doublet_remove <-
  function(h5_mat_path = NULL, gz_mat_path = NULL, sample_id = NULL, doublet_rate = NULL) {
    ...
    # add QC
    data <- PercentageFeatureSet(data, pattern = "^mt-", col.name = "percent.mt") # mitogene
    data <- PercentageFeatureSet(data, pattern = "^Rp[sl][[:digit:]]", col.name = "percent.rb") #ribosome
    data <- PercentageFeatureSet(data, features = CaseMatch(c("Hba-a1","Hba-a2","Hbb-bt","Hbb-bs","Hbb-b1","Hbb-b2","Hbb-bh0","Hbb-bh1","Hbb-bh2","Hba-x","Hbb-y","Hbq1a","Hbq1b"), rownames(data)), col.name = "percent.HB")   #erythrocyte
    ...
    data <- subset(data, subset = nFeature_RNA > 200 & nFeature_RNA <7500  & percent.mt < 10 & percent.rb < 20 & percent.HB < 1)
    ...
    sweep.res <- paramSweep_v3(data, PCs = 1:15, sct = FALSE)
    ...
    data <- doubletFinder_v3(data, PCs = 1:15, pN = 0.25, pK = pk, nExp = nExp_poi, reuse.pANN = FALSE, sct = FALSE)
    data <- doubletFinder_v3(data, PCs = 1:15, pN = 0.25, pK = pk, nExp = nExp_poi.adj, reuse.pANN = paste0("pANN_0.25_", pk, "_", nExp_poi), sct = FALSE)
